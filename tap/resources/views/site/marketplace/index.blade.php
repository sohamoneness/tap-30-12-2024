        ...
        <div class="row justify-content-center text-center">
            <div class="col-lg-8 col-md-6 col-12 mb-4">
                <div class="markets-banner-content freelance-market-banner-content">
                    ...
                    <p class="px-0 px-sm-5">{{$marketplace_page_content->header_short_description}}</p>
                        ...
                        <form action="">
                            ...
                            <select name="country" id="country">
                                ...
                                @forelse ($countries as $country)
                                    @if($country != '')
                                        <option value="{{$country}}" {{$country == trim(request()->input('country')) ? 'selected' : ''}}>{{ucwords($country)}}</option>
                                    @endif
                                @empty
                                    <option value="">No user have any country!</option>
                                @endforelse
                            </select>
                            <select name="language" id="language">
                                <option value="" selected hidden>Select language</option>
                                @forelse ($languages as $language)
                                    <option value="{{$language->id}}" @if(request()->input('language') == $language->id ) selected @endif >{{$language->name}}</option>
                                @empty
                                    <option value="">No user have any language!</option>
                                @endforelse
                            </select>
                            <select name="occupation" id="occupation">
                                ...
                                @forelse ($occupations as $occupation)
                                    @if($occupation != '' || $occupation != null)
                                        <option value="{{$occupation}}" {{$occupation == trim(request()->input('occupation')) ? 'selected' : ''}}>{{ucwords($occupation)}}</option>
                                    @endif
                                @empty
                                    <option value="">No user have any occupation!</option>
                                @endforelse
                            </select>
    ...
    <div class="container">
        <div class="row align-items-center">
            <div class="col-12 text-center text-lg-start">
                <div class="recommended-writers-left">
                    <h6>Writers </h6>
                    <p>{{count($writers)}} writer found!</p>
                </div>
            </div>
        ...
        <div class="row mt-4">
            @forelse ($writers as $data)
                <div class="col-lg-3 col-md-4 col-12 mb-lg-4 mb-md-4 mb-4">
                    <div class="recommended-writers-content">
                        <div class="content-top">
                            @if($data->image)
                            <figure>
                                <img src="{{ asset($data->image)}}" height="60px" width="60px" class="" alt="">
                            </figure>
                            @else
                            <figure>
                                <img src="{{ asset('images/user.webp')}}" height="60px" width="60px" class="" alt="">
                            </figure>
                            @endif
                        ...
                        <div class="content-mid">
                            <ul class="list-unstyled p-0 m-0 mt-2" id="showLessContent_{{$data->id}}">
                        ...
                        <div class="content-btm">
                            <a href="{{route('sitenew.marketplace.details', $data->slug)}}">
                                get started now
        ...
        <div class="row align-items-center">
            <div class="col-12 text-center text-lg-start">
                <div class="recommended-writers-left">
                    ...
                    <p>{{$recomended_writers->count()}} writer found!</p>
                </div>
            </div>
            <div class="col-lg-6 col-md-6 col-12 text-center text-lg-end">
                <div class="recommended-writers-right">
                ...
                <div class="col-lg-4 col-md-4 col-12 mb-lg-4 mb-md-4 mb-4 br">
                    <div class="recommended-writers-content">
                        ...
                        <div class="content-mid mt-3">
                            <p>{{substr($item->short_desc,0,15)}}</p>
                            <ul class="list-unstyled p-0 m-0 mt-2" id="showLessContent_{{$item->id}}">
                                @for($i=0; $i<count(explode(',',$item->categories)); $i++)
                                    @if($i<5)
                                        @if(explode(',',$item->categories)[$i] != '')
                                            <li>{{explode(',',$item->categories)[$i]}}</li>
                                        @endif
                                    @endif
                                @endfor
                                @if(count(explode(',',$item->categories)) > 2)
                                    <li class="showMore" onclick="showMore({{$item->id}})" style="cursor: pointer;">+ {{count(explode(',',$item->categories)) - 5}} more</li>
                                @endif
                            </ul>
                            <ul class="list-unstyled p-0 m-0 d-none" style="flex-flow: wrap;" id="showMoreContent_{{$item->id}}">
                                @for($i=0; $i<count(explode(',',$item->categories)); $i++)
                                    @if(explode(',',$item->categories)[$i] != '')
                                        <li class="my-1">{{explode(',',$item->categories)[$i]}}</li>
                                    @endif
                                @endfor
                            </ul>
                        </div>
                        <div class="content-btm">
                            ...
                            <a href="{{route('sitenew.marketplace.details', $item->slug)}}">
                                get started now
                                <img src="{{ asset('frontend/img/arrow-right-freelance.png')}}" alt="">
                            </a>
                        </div>
        ...
        <div class="row justify-content-center">
            <div class="col-sm-7">
                <div class="title_area text-center">
                    ...
                    <p>Discover answers to all of the top questions publishers want to know. Did we miss something? Get in touch to ask us a question.</p>
                </div>
            </div>
        </div>
        ...
        <div class="row justify-content-center">
            <div class="col-sm-8">
                ...
                <div class="accordian_box">
                    <h5><span>{{$i}}.</span> {{ $mfaq->question }}</h5>
