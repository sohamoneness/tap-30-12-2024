                        ...
                        <span class="top-form-btn">
                            <form action="{{ route('front.project.task.store') }}" method="POST" role="form" enctype="multipart/form-data">@csrf
                                <div class="tile-body">
                                    ...
                                    <div class="form-group">
                                        <label class="control-label" for="deadline">Deadline <span class="m-l-5 text-danger">*</span>(task deadline must be before of equal to project deadline)</label>

                                        <input class="form-control @error('deadline') is-invalid @enderror" type="date" name="deadline" id="deadline" value="" min="{{ now()->format('Y-m-d') }}" max="{{date('Y-m-d', strtotime($project->deadline)) }}"/>
