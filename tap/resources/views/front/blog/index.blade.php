                    ...
                    <li>
                        <label>
                            <form action="{{request()->fullUrl()}}" method="GET">
                                <input class="d-none" type="checkbox" onclick="$(this).parent().submit()" name="category" value="all" {{ (request()->input('category') == 'all') ? 'checked' : '' }}>
                            </form>
                            <span class="{{ ((request()->input('category') ?? 'all') == 'all') ? 'bg-success' : '' }}">All</span>
                        </label>
                    </li>
                 @foreach($cat as $key => $data)
                    <li>
                        <label>
                            ...
                            <span class="{{ (request()->input('category') == $data->slug) ? 'bg-success' : '' }}">{{  ucwords($data->title) }}</span>
                        </label>
                    </li>
                 @endforeach
                </ul>
        ...
        <div class="row blog_list">
            @foreach($blogs as $key => $blog)
                <div class="col-12 col-lg-4 col-md-6 mb-3 blog-list-panel" id="blog{{implode('-',explode(',',$blog->article_category_id))}}">
                        <div class="card">
                            <a href="{{ route('front.article.details',$blog->slug) }}">
