    ...
    {  
        //dd($request->type);
         if (auth()->guard('web')->check()) {
            ...
            if (isset($request->code) || isset($request->keyword) || isset($request->price)|| isset($request->location)){
               ...
               if(!empty($request->type)){
                ...
                $event=Event::where('type','like','%'.$cat_id.'%')->where('status',1)->orderby('title')->paginate(12);
                //dd($event);
                }else{
                    $event=Event::where('status',1)->where('type','like','%'.$cat[0]->id.'%')->orderby('title')->paginate(12);
                }
            }
