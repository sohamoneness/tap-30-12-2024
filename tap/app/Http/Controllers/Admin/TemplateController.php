...
class TemplateController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        $extention = $params['file']->extension();
         
          
        if($params['type'] == '2'){
             if($extention != 'pdf') {
               return $this->responseRedirectBack('Please upload only pdf file.', 'error', true, true);
          }
        }
        if($params['type'] == '3'){
             if($extention != 'ppt') {
               return $this->responseRedirectBack('Please upload only ppt file.', 'error', true, true);
          }
        }
        if($params['type'] == '4'){
             if($extention != 'xls') {
               return $this->responseRedirectBack('Please upload only xls file.', 'error', true, true);
          }
        }
