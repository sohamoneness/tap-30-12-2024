...
class BlogController extends BaseController
{
    ...
    public function edit($id)
    {
        ...
        $articlecat = $this->BlogRepository->getArticlecategories();
        $articlesubcat = $this->BlogRepository->getArticlesubcategories();
        $articleContent= $this->BlogRepository->getArticlecontent($id);
        $this->setPageTitle('Blog', 'Edit Blog : ' . $targetarticle->title);
        return view('admin.article.edit', compact('targetarticle', 'articlecat', 'articlesubcat','articleContent'));
    }
    ...
    {
        $this->setPageTitle('Blog', 'Create Blog');
         $articlecat = ArticleContent::where('blog_id',$request->id)->get();
        return view('admin.article.content.create', compact('articlecat','request'));
    }
    ...
    public function contentStore(Request $request)
    {
        ...
        $article->slug = slugGenerate($request['title'],'article_contents');
        ...
        return $this->responseRedirectBack('Blog content has been created successfully', 'success', false, false);
    }
     ...
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View
     */
    public function contentEdit($id,Request $request)
    {
        $targetarticle = ArticleContent::findOrfail($id);
        $this->setPageTitle('Blog', 'Edit Blog : ' . $targetarticle->title);
        return view('admin.article.content.edit', compact('targetarticle','request'));
    }
    ...
    public function contentUpdate(Request $request)
    {
       ...
       // $params = $request->except('_token');
        ...
        $targetblog = ArticleContent::findOrFail($request->id);
        $targetblog->title = $request->title;
        //$article->slug = slugGenerate($request['title'],'article_contents');
        $targetblog->type = $request->type;
        $targetblog->content = $request->content;
        $targetblog->save();
    ...
    {
        $targetarticle = ArticleContent::where('id',$id)->first();
        $this->setPageTitle('Blog', 'Blog Details : ' . $targetarticle->title);
        return view('admin.article.content.details', compact('targetarticle'));
    }

    public function contentDelete($id)
    {
        $targetblog = ArticleContent::findOrFail($id);
        $targetblog->delete();
        if (!$targetblog) {
            return $this->responseRedirectBack('Error occurred while deleting Blog.', 'error', true, true);
        }
    ...
    public function search(Request $request)
    {
        ...
        $data = Article::select('title', 'articles.id', 'image')
        ...
        if (count($data) > 0) {
            ...
            return response()->json([
                'status' => 400,
                'message' => 'No records found. Please enter an existing title.'
            ]);
        }
    }

    public function articleToggle(Request $request)
    {
        $validate = Validator::make($request->all(), [
            'toolId' => 'required|integer|min:1',
        ...
        // check first
        $check = ToolArticle::where('tool_id', $request->toolId)
        ...
        if (!empty($check)) {
            ...
            $new = new ToolArticle();
            $new->tool_id = $request->toolId;
            $new->article_id = $request->articleId;
            $new->save();
            $resp = true;
        }
    ...
    public function articleFetch(Request $request)
    {
        $validate = Validator::make($request->all(), [
            'toolId' => 'required|integer|min:1'
        ]);
        ...
        $data = ToolArticle::where('tool_id', $request->toolId)->with('articleDetails')->latest('id')->get();
